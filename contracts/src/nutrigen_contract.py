...
class NutrigenContract(gl.Contract):
    ...
    def _assert_no_predecided_verdict(self, text: str) -> None:
        ...
        forbidden = [
            ...
            '"cost_score"', "'cost_score'", "cost_score:",
            '"nutrition_score"', "'nutrition_score'", "nutrition_score:",
    ...
    def _create_optimization_request(
        ...
        self._assert_no_predecided_verdict(
            objective_summary + " " + current_feeding_summary + " " +
            available_feed_summary + " " + candidate_ration_summary + " " +
            nutrient_analysis_summary + " " + cost_constraint_summary + " " +
            supply_constraint_summary + " " + health_context_summary
        )
        ...
        self.farm_request_index[farm_id] = self._append(
            self.farm_request_index.get(farm_id, ""), final_request_id)
        self.batch_request_index[batch_id] = self._append(
            self.batch_request_index.get(batch_id, ""), final_request_id)
        self.advisor_request_index[advisor_id] = self._append(
            self.advisor_request_index.get(advisor_id, ""), final_request_id)

        self._record_audit(
            farm_id, final_request_id,
            "FEED_OPTIMIZATION_REQUEST_CREATED", self._sender(),
            "Livestock feed optimization request submitted",
            evidence_manifest_hash, submitted_at,
        )
...
""",
    ...
    def _adjudicate_feed_request(self, request_id: str, adjudicated_at: str) -> str:
        ...
        review = self._run_consensus_feed_review(
            request_record, farm, batch, advisor, standard_packet, ingredient_packet)
        ...
        self._record_audit(
            farm_id, request_id,
            "GENLAYER_FEED_OPTIMIZATION_DECISION", "GENLAYER_CONSENSUS",
            "Consensus livestock feed verdict: " + verdict,
            decision_id, adjudicated_at,
        )

        return self._json(decision_record)

    def _update_reviewer_reputation(
        self, reviewer: str, farm_id: str, accepted: bool, reviewed_at: str) -> None:
    ...
    def transfer_ownership(self, new_owner: str, updated_at: str) -> None:
        ...
        self._record_audit("", "", "OWNERSHIP_TRANSFERRED", previous,
            "Contract ownership transferred", new_owner, updated_at)
    ...
    @gl.public.write
    def create_farm(self, farm_id: str, name: str, farm_type: str,
                    location_context: str, metadata_hash: str, created_at: str) -> str:
        self._require_not_paused()
        ...
        self.farm_index["all"] = self._append_unique(self.farm_index.get("all", ""), final_farm_id)

        self._record_audit(final_farm_id, "", "FARM_CREATED", self._sender(),
            "Farm created for livestock feed optimization", metadata_hash, created_at)

        return final_farm_id

    @gl.public.write
    def add_farm_role(self, farm_id: str, wallet: str, role: str, added_at: str) -> None:
        self._require_not_paused()
        ...
        self._record_audit(farm_id, "", "FARM_ROLE_ADDED", self._sender(),
            "Added " + final_role + " role for " + wallet.lower(), wallet.lower(), added_at)

    @gl.public.write
    def remove_farm_role(self, farm_id: str, wallet: str, removed_at: str) -> None:
        self._require_not_paused()
        self._require_farm_exists(farm_id)
        self._require_farm_owner_or_admin(farm_id)
        ...
        self._record_audit(farm_id, "", "FARM_ROLE_REMOVED", self._sender(),
            "Removed farm role for " + wallet.lower(), wallet.lower(), removed_at)

    @gl.public.write
    def set_farm_status(self, farm_id: str, status: str, updated_at: str) -> None:
        self._require_not_paused()
        self._require_farm_owner_or_admin(farm_id)
        ...
        self._record_audit(farm_id, "", "FARM_STATUS_UPDATED", self._sender(),
            "Farm status set to " + final_status, final_status, updated_at)

    @gl.public.write
    def set_farm_optimization_config(
        self, farm_id: str,
        min_approve_nutrient_adequacy: u256, min_approve_suitability: u256,
        min_approve_safety: u256, min_approve_availability: u256,
        min_approve_practicality: u256, max_approve_risk: u256,
        auto_review_risk: u256, auto_reject_risk: u256, updated_at: str,
    ) -> None:
        self._require_not_paused()
        self._require_farm_owner_or_admin(farm_id)

        for value in [min_approve_nutrient_adequacy, min_approve_suitability,
                      min_approve_safety, min_approve_availability, min_approve_practicality,
                      max_approve_risk, auto_review_risk, auto_reject_risk]:
            if value > u256(100):
        ...
        self._record_audit(farm_id, "", "FARM_OPTIMIZATION_CONFIG_UPDATED", self._sender(),
            "Feed optimization thresholds updated",
            self._json(farm["optimization_config"]), updated_at)
    ...
    def register_feed_advisor(
        self, advisor_id: str, farm_id: str, name: str,
        ...
        self.farm_advisor_index[farm_id] = self._append_unique(
            self.farm_advisor_index.get(farm_id, ""), final_advisor_id)

        self._record_audit(farm_id, "", "FEED_ADVISOR_REGISTERED", self._sender(),
            "Feed advisor registered: " + final_advisor_id, metadata_hash, registered_at)

        return final_advisor_id

    @gl.public.write
    def set_feed_advisor_status(self, advisor_id: str, status: str, updated_at: str) -> None:
        self._require_not_paused()
        ...
        self._record_audit(farm_id, "", "FEED_ADVISOR_STATUS_UPDATED", self._sender(),
            "Feed advisor " + advisor_id + " status set to " + final_status, final_status, updated_at)
    ...
    def register_livestock_batch(
        self, batch_id: str, farm_id: str, species: str, breed_summary: str,
        production_stage: str, production_goal: str, head_count: str,
        weight_summary: str, health_status_summary: str, feeding_constraints: str,
        metadata_hash: str, registered_at: str,
    ) -> str:
        self._require_not_paused()
        ...
        self.farm_batch_index[farm_id] = self._append_unique(
            self.farm_batch_index.get(farm_id, ""), final_batch_id)

        self._record_audit(farm_id, "", "LIVESTOCK_BATCH_REGISTERED", self._sender(),
            "Livestock batch registered", metadata_hash, registered_at)

        return final_batch_id

    @gl.public.write
    def update_livestock_batch_summary(
        self, batch_id: str, production_goal: str, head_count: str,
        weight_summary: str, health_status_summary: str, feeding_constraints: str,
        metadata_hash: str, updated_at: str,
    ) -> None:
        self._require_not_paused()
        ...
        self._record_audit(farm_id, "", "LIVESTOCK_BATCH_UPDATED", self._sender(),
            "Livestock batch context updated", metadata_hash, updated_at)

    @gl.public.write
    def set_livestock_batch_status(self, batch_id: str, status: str, updated_at: str) -> None:
        self._require_not_paused()
        ...
        self._record_audit(farm_id, "", "LIVESTOCK_BATCH_STATUS_UPDATED", self._sender(),
            "Livestock batch status set to " + final_status, final_status, updated_at)
    ...
    def register_feed_ingredient(
        self, ingredient_id: str, farm_id: str, name: str, category: str,
        nutrient_profile_summary: str, safety_summary: str,
        availability_summary: str, cost_summary: str,
        metadata_hash: str, registered_at: str,
    ) -> str:
        self._require_not_paused()
        self._require_farm_reviewer_or_admin(farm_id)
        ...
        self.farm_ingredient_index[farm_id] = self._append_unique(
            self.farm_ingredient_index.get(farm_id, ""), final_ingredient_id)

        self._record_audit(farm_id, "", "FEED_INGREDIENT_REGISTERED", self._sender(),
            "Feed ingredient registered: " + name, metadata_hash, registered_at)

        return final_ingredient_id

    @gl.public.write
    def update_feed_ingredient(
        self, ingredient_id: str, nutrient_profile_summary: str,
        safety_summary: str, availability_summary: str, cost_summary: str,
        metadata_hash: str, updated_at: str,
    ) -> None:
        self._require_not_paused()
        ...
        self._record_audit(farm_id, "", "FEED_INGREDIENT_UPDATED", self._sender(),
            "Feed ingredient details updated", metadata_hash, updated_at)

    @gl.public.write
    def set_feed_ingredient_status(self, ingredient_id: str, status: str, updated_at: str) -> None:
        self._require_not_paused()
        ...
        final_status = self._normalise_status(
            status, "ACTIVE|UNAVAILABLE|SUSPENDED|ARCHIVED", "feed ingredient status")
        ingredient["status"] = final_status
        ingredient["updated_at"] = updated_at
        self.feed_ingredients[ingredient_id] = self._json(ingredient)

        self._record_audit(farm_id, "", "FEED_INGREDIENT_STATUS_UPDATED", self._sender(),
            "Feed ingredient status set to " + final_status, final_status, updated_at)
    ...
    def publish_feed_standard_version(
        self, farm_id: str, standard_id: str, version: str, title: str,
        species_scope: str, production_stage_scope: str, severity: str,
        nutrient_target_rules: str, ingredient_limit_rules: str,
        toxin_and_anti_nutrient_rules: str, health_escalation_rules: str,
        cost_and_availability_rules: str, standard_hash: str,
        metadata_hash: str, published_at: str,
    ) -> str:
        self._require_not_paused()
        self._require_farm_owner_or_admin(farm_id)
        ...
        self.farm_standard_index[farm_id] = self._append_unique(
            self.farm_standard_index.get(farm_id, ""), standard_id)
        self.standard_version_index[self._key2(farm_id, standard_id)] = self._append_unique(
            self.standard_version_index.get(self._key2(farm_id, standard_id), ""), version)

        self._record_audit(farm_id, "", "FEED_STANDARD_VERSION_PUBLISHED", self._sender(),
            "Feed standard " + standard_id + " version " + version + " published",
            standard_hash, published_at)

        return standard_key

    @gl.public.write
    def set_current_feed_standard_version(
        self, farm_id: str, standard_id: str, version: str, updated_at: str) -> None:
        self._require_not_paused()
        self._require_farm_owner_or_admin(farm_id)
        ...
        self._record_audit(farm_id, "", "CURRENT_FEED_STANDARD_UPDATED", self._sender(),
            "Current feed standard " + standard_id + " set to version " + version,
            self._key3(farm_id, standard_id, version), updated_at)

    @gl.public.write
    def set_feed_standard_version_status(
        self, farm_id: str, standard_id: str, version: str,
        ...
        self._record_audit(farm_id, "", "FEED_STANDARD_VERSION_STATUS_UPDATED", self._sender(),
            "Feed standard " + standard_id + " version " + version +
            " status set to " + final_status, final_status, updated_at)
    ...
    def submit_feed_optimization_request(
        self, request_id: str, farm_id: str, batch_id: str, advisor_id: str,
        standard_ids_csv: str, ingredient_ids_csv: str, objective_summary: str,
        ...
        return self._create_optimization_request(
            request_id, farm_id, batch_id, advisor_id, standard_ids_csv,
    ...
    @gl.public.write
    def adjudicate_feed_optimization(self, request_id: str, adjudicated_at: str) -> str:
    ...
    def submit_and_optimize_feed(
        ...
        health_context_summary: str, environment_context_summary: str,
        evidence_manifest_hash: str, ration_hash: str,
        submitted_at: str, expires_at: str, adjudicated_at: str,
    ) -> str:
    ...
    def human_feed_review_decision(
        self, request_id: str, final_verdict: str, review_reason: str,
        review_evidence_hash: str, reviewer_notes: str, decided_at: str,
    ) -> str:
        self._require_not_paused()
        ...
        self.reviewer_decision_index[self._key2(farm_id, self._sender())] = self._append(
            self.reviewer_decision_index.get(self._key2(farm_id, self._sender()), ""), request_id)

        self._record_audit(farm_id, request_id, "HUMAN_FEED_REVIEW_DECISION", self._sender(),
            "Human livestock feed review decision: " + verdict, review_evidence_hash, decided_at)

        return self._json(review_record)

    @gl.public.write
    def mark_feed_plan_activated(
        self, request_id: str, activation_hash: str,
        ...
        self._record_audit(farm_id, request_id, "FEED_PLAN_ACTIVATED", sender,
            "Approved livestock feed plan activated", activation_hash, activated_at)

        return self._json(activation_record)

    @gl.public.write
    def mark_feed_plan_blocked(self, request_id: str, block_reason: str, blocked_at: str) -> None:
        self._require_not_paused()
        ...
        self._record_audit(farm_id, request_id, "FEED_PLAN_BLOCKED", self._sender(),
            "Livestock feed plan manually blocked", block_reason, blocked_at)
